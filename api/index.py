...
def get_timestamp_days_ago(days):
    ...
    # This has been updated to use datetime.now(UTC) to avoid deprecation warnings.
...
@app.route("/")
def index():
    # This line attempts to render a file named "index.html" from a "templates" folder.
    try:
        return render_template("index.html")
    except Exception as e:
        # This will show a more helpful error message if the file can't be found.
...
def upload_resume():
    ...
    if file:
        ...
        candidate = {
            "name": candidate_name,
            "email": email_from_text,
            "phone": phone_from_text,
            "filename": filename,
            "sections": sections
        }
        
        return jsonify({"candidates": [candidate]})
...
def chat():
    """Handles chat messages and uses the LLM to respond."""
    data = request.json
    ...
    # System prompt to define the assistant's persona and capabilities
    system_prompt = """
    ...
    Job Description Prompting: If user asks for a JD, always ask for:
    Years of experience
